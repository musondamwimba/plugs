...
serve(async (req) => {
  ...
  try {
    ...
    if (payment_method === 'Mobile Money' && !phone_number) {
      throw new Error('Phone number required for Mobile Money');
    }

    if (payment_method === 'Bank Transfer' && !account_number) {
      throw new Error('Account number required for Bank Transfer');
    }
    ...
    // TODO: Integrate with actual payment provider (MTN, Airtel, Bank API)
    // For now, simulate sending verification code
    console.log(`Verification code for ${phone_number || account_number}: ${verificationCode}`);
